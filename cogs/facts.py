...
class Facts(commands.Cog):
    ...
    async def set_fact_channel(self, ctx, channel: discord.TextChannel):
        helper.single_SQL("UPDATE Guilds SET FactChannelID={0} WHERE ID={1}".format(channel.id, ctx.guild_id))
    ...
    async def toggle_facts(self, ctx):
        helper.single_SQL("UPDATE Guilds SET FactChannelID=NULL WHERE ID={0}".format(ctx.guild_id))
