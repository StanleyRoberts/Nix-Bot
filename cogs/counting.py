...
class Counting(commands.Cog):
    ...
    async def set_fail_role(self, ctx, role: discord.Role):
        single_SQL("UPDATE Guilds SET FailRoleID=%s WHERE ID=%s", (role.id, ctx.guild_id))
    ...
    async def set_counting_channel(self, ctx, channel: discord.TextChannel):   
        single_SQL("UPDATE Guilds SET CountingChannelID=%s WHERE ID=%s", (channel.id, ctx.guild_id))
    ...
    async def get_highscore(self, ctx):
        highscore = single_SQL("SELECT HighScoreCounting FROM Guilds WHERE ID = %s", (ctx.guild.id,))
