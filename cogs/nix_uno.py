...
def allCards():                                      # Sets all available cards
    ...
    wilds = ['WILD Draw 4', 'WILD']                  # want to add specific cards like a "flame" card.. bc its Nix Uno
    for c in colors:
        for v in values:
            cardVal = '{} {}'.format(c, v)
            deck.append(cardVal)
...
class NixUno(commands.Cog):
    ...
    @commands.slash_command(name="nix_uno", description="Starts a game of NixUno and slect players who should participate")
    async def start_game(self, ctx,
        ...
        await ctx.respond(player1.mention + ' ' + player2.mention + ' ' + player3.mention + ' are playing right now <:NixSip:1025434211986984971>')
...
class MyView(discord.ui.View): # Create a class called MyView that subclasses discord.ui.View
    @discord.ui.button(label="Start Game", style=discord.ButtonStyle.green) # Create a button with the label "😎 Click me!" with color Blurple
