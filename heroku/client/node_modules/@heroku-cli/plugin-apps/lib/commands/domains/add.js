...
const multiple_sni_feature_1 = require("../../lib/multiple-sni-feature");
...
class DomainsAdd extends command_1.Command {
    ...
    async run() {
        ...
        const { hostname } = args;
        const multipleSniEndpointsEnabled = await multiple_sni_feature_1.default(this.heroku, flags.app);
        const domainCreatePayload = {
            hostname,
        };
        let certs = [];
        cli_ux_1.default.action.start(`Adding ${color_1.color.green(domainCreatePayload.hostname)} to ${color_1.color.app(flags.app)}`);
